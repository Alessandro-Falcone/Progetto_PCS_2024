...
bool letturaMesh(DFN &Fract, PolygonalMesh &mesh){
    ...
    if(!letturaDatiCell1Ds(Fract, mesh)){
        return false;
    }else{

    }

    if(!letturaDatiCell2Ds(Fract, mesh)){
        return false;
    }else{
        ...
        if(numeroTraccePerFrattura > 0){
            ...
            for(unsigned int t = 0; t < idTracciaPassante.size(); t++){
                ...
                if((idTracciaPassante.size() > 0 && idTracciaPassante.size() < 2) && (idTracciaNonPassante.size() < 1)){
                    ...
                    mesh.NumberCell0Ds = idCell0Ds + matrPuntiDiIntersezioneTracciaPassante.rows();
                    mesh.NumberCell1Ds = idCell1Ds + matrPuntiDiIntersezioneTracciaPassante.rows();
                    ...
                    for(unsigned int i = 0; i < 2; i++){
                        ...
                        mesh.IdCell1Ds.push_back(idCell1Ds);
                        idCell0Ds++;
                        idCell1Ds++; // aumento di uno k in quanto i punti di intersezione delle tracce sono sempre 2
                    }
                    ...
                    idCell0Ds = 0; // identificativo celle 0Ds
                    idCell1Ds = 0; // identificativo celle 1Ds
                    idCell2Ds = 0;
                    nVertices = 0;
                    nEdges = 0;
                    mesh.IdCell0Ds.reserve(2);
                    ...
                    mesh.NumberCell0Ds = idCell0Ds;
                    mesh.NumberCell1Ds = idCell1Ds;
                    cout << "NumberCell0Ds: " << mesh.NumberCell0Ds << endl;
                    cout << "NumberCell1Ds: " << mesh.NumberCell1Ds << endl;
            ...
            for(unsigned int t = 0; t < idTracciaNonPassante.size(); t++){
                ...
                if((t < 1) && (numeroTraccePassantiPerFrattura > 0) && idTracciaNonPassante.size() > 0){
                    ...
                    Vector3d proiezionePDiInterSuTracciaPass = matrPuntiDiIntersezionePrecTracciaPassante.row(0) + t * diffPuntiDiIntersezioneTracciaPassante.transpose();
                    if(puntoDiIntersezione1aSinistra && puntoDiIntersezione2aSinistra){
                        ...
                        mesh.NumberCell1Ds = idCell1Ds + matrPuntiDiIntersezionePrecTracciaPassante.rows();
                        cout << "NumberCell0Ds: " << mesh.NumberCell0Ds << endl;
                        cout << "NumberCell1Ds: " << mesh.NumberCell1Ds << endl;
